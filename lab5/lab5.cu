...
__device__ float func1(float x, float y, float z)
{
  ...
  float a = (0.5f * sqrtf(15.0f / M_PI)) * (0.5f * sqrtf(15.0f / M_PI)) * z * z * y * y * sqrtf(1.0f - z * z / RADIUS / RADIUS) / RADIUS / RADIUS / RADIUS / RADIUS;
...
void init_texture(float *df_h)
{
  ...
  CUDA_CHECK_RETURN(cudaMalloc3DArray(&df_Array, &channelDesc, volumeSize));
  ...
  CUDA_CHECK_RETURN(cudaMemcpy3D(&cpyParams));
  ...
  CUDA_CHECK_RETURN(cudaBindTextureToArray(df_tex, df_Array, channelDesc));
  return;
}
void release_texture()
{
  CUDA_CHECK_RETURN(cudaUnbindTexture(df_tex));
  CUDA_CHECK_RETURN(cudaFreeArray(df_Array));
  return;
}
...
int main()
{
  ...
  CUDA_CHECK_RETURN(cudaMalloc((void **)&values, sizeof(float) * FGSIZE * FGSIZE * FGSIZE));
  CUDA_CHECK_RETURN(cudaMemcpy(values, arr, sizeof(float) * FGSIZE * FGSIZE * FGSIZE, cudaMemcpyHostToDevice));
  init_texture(arr);
